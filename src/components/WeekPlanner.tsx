...
const WeekPlanner = () => {
    ...
    useEffect(() => {
        const fetchAllTasks = async () => {
            ...
            for (const day of weekDays) {
                ...
                // Нормализуем дату: приводим к YYYY-MM-DD
                const normalizedTasks = dayTasks.map((task: any) => ({
                    ...task,
                    date: task.date ? new Date(task.date).toISOString().split('T')[0] : null
                }))
    ...
    const handleCreateTask = async (date: Date) => {
        ...
        try {
            ...
            console.log('Created task:', newTask) // todo log
    ...
    const handleToggleTask = async (task: Task) => {
        try {
            ...
            // Нормализуем дату (на случай, если бэкенд вернёт ISO)
            const normalizedUpdated = {
                ...updated,
                date: updated.date ? new Date(updated.date).toISOString().split('T')[0] : null
            }
            // Обновляем задачу в списке, а не удаляем её
                        ...
                        <div key={index} className="day-column">
                            ...
                            <div className="tasks-list">
                                ...
                                <div className="add-task">
                                    ...
                                    />
                                    <button onClick={() => handleCreateTask(day)} className="add-task-btn">+</button>
                                </div>
                            </div>
                        </div>
                    )
